...
export async function registrar_compra(req,res){
    ...
    if(req.session.login){
        ...
        compra_parse.map(async (item)=>{
            ...
            await cliente.query(`INSERT INTO compra (id_compra, id_usuario, id_produto, produto, categoria, quantidade, preco, data_compra) VALUES(${id_compra},${req.session.login.id} , 
            ${produto.rows[0].id}, '${item.produto}', '${item.categoria}', ${item.quantidade}, ${valor_total}, 'NOW()')`)
